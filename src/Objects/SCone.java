...
public class SCone extends SObject{
    //make a new height parameter over the sphere
    private float radius;
    private float height;
    private int slices;
    private int stacks;

    //generator functions/setters
    public SCone(){
        super();
        init();
        update();
    ...
    public SCone(float radius, float height, int slices, int stacks){
        super();
        this.radius = radius;
        this.height = height;
        this.slices = slices;
    ...
    protected void genData() {
        ...
        for(i=1;i<stacks;i++){
            for(j=0;j<=slices;j++){
                //following equation  (h-u)/h * rcos(theta)
                normals[3*k] = cos(deltaLong*j);
                //following equation  (h-u)/h * rsin(theta)
                normals[3*k+1] = sin(deltaLong*j);
                //following equation  (h-u)/h
                normals[3*k+2] = 1;
                //Convoluted way of getting the current height
                vertices[3*k] = ((height - i*height/stacks)/height)*radius*normals[3*k];
                vertices[3*k+1] = ((height - i*height/stacks)/height)*radius*normals[3*k+1];
                vertices[3*k+2] = (i*height/stacks)*normals[3*k+2];
